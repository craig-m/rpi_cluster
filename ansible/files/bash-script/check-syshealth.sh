#!/usr/bin/env bats
# check this R-Pi node is OK with bats
...
@test "no deleted file procs" {
  delprcs=$(/usr/bin/sudo lsof | grep deleted | wc -l)
  [ "$delprcs" = "0" ]
}

@test "no executable files in tmp" {
  exetmp=$(/usr/bin/sudo find /tmp/ -executable -type f | wc -l)
  [ "$exetmp" = "0" ]
}
